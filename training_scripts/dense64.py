...
# The set_padding function is not causing a significant computational effort, but is redundant if some slight changes are applied.
# Explanation: Instead of this function, change the min_range & max_range variable in the voxelize_in_model function to the desired grid size.
# After changing this and removing the set_pedding implementation check if parts of the training need slight changes (i.e. if the
# information flow between the functions still works).
