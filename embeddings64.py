...
lmodel = AE(n_channels=1,n_classes=1, base_channels=32, num_blocks=3, transpose=True, residual=True).to(DEVICE)
loptimizer = torch.optim.AdamW(lmodel.parameters(), lr=3e-3)

model_dense64, optim_dense64, n_iter_dense64 = load_model(lmodel, loptimizer, name="model_final/models/14k/64_bs50_nLat0_a0.65g1-lr0.003_F1")
print(n_iter_dense64)

out_dir = join(path, 'df_latent_label_64dense.pkl') 
print(out_dir)

latent_embeddings_64dense = get_embeddings_encdec(model_dense64, dataset_train, id_list, grid_size=grid_size)
# for each element in id_list, load neuron run through L-Block, extract latent cube, flatten, add to list

print(latent_embeddings_64dense.shape)

latent_df_dense = pd.DataFrame(data={'latent_emb':list(latent_embeddings_64dense),'segment_split': id_list})
...
model_nLat64, optim_nLat64, n_iter_nLat64 = load_model(lmodel, loptimizer, name="model_final/models/14k/64_bs50_nLat10_a0.65g1-lr0.003_F1")
print(n_iter_nLat64)

out_dir = join(path, 'df_latent_label_64nLat.pkl') 
print(out_dir)

latent_embeddings_64nLat = get_embeddings_encdec(model_nLat64, dataset_train, id_list, grid_size=grid_size)

print(latent_embeddings_64nLat.shape)

latent_df_nLat = pd.DataFrame(data={'latent_emb':list(latent_embeddings_64nLat),'segment_split': id_list})
